...
- **Sprint 6**: Khắc phục lỗi Facebook Pages `CreatePostWithPhotos` trên Make.com: Đã bổ sung thuộc tính `type: 'url'` cho mảng `facebook_photos` để khớp schema Make.com, đồng thời nâng cấp thuật toán trích xuất ảnh thông minh (`extractBlogImages`): Tự động ưu tiên lấy 100% ảnh sản phẩm thật từ bài viết & CSDL kho hàng, loại bỏ tuyệt đối các ảnh chụp màn hình Admin UI.
