...
- **Sprint 8 (Hoàn thành)**: Giải quyết dứt điểm lỗi Make.com Webhook & Facebook Pages. Khắc phục lỗi `324 OAuthException` bằng cách buộc Webhook `Redetermine data structure` để parse mảng ảnh chuẩn. Xóa bỏ logic ép ảnh mồi Unsplash (Hardcoded) để trả về đúng ảnh Thumbnail gốc. Khắc phục lỗi ngập lụt Queue (Make.com Rate Limit & deactivated scenario) bằng cách thêm delay 5 phút vào route `make-share-all`. Chỉnh sửa lỗi `finalLink` bị hardcode sai domain ở DHTK. Cả hai dự án `DHTK` và `thohong` đều đã đồng bộ code thành công.
...
- **Sprint 10 (Hoàn thành)**: Vá triệt để các lỗ hổng bảo mật mảng đồng bộ POS: Khóa route `/api/pos/sync` bằng `authenticateToken` chống DDoS & Rate Limit POS, đồng bộ 100% chuẩn bảo mật với Thỏ Hồng Store.
...
- **Sprint 12 (Hoàn thành)**: Khắc phục triệt để lỗi CORS ALLOWED_ORIGINS bị thiếu `donghangtietkiem.com` gây lỗi 500 HTML trên trình duyệt, đồng bộ cấu hình `vercel.json` chuẩn Vercel Hobby, bổ sung `await` cho `triggerMakeWebhook` trong chu kỳ tự động viết bài Blog AI để bài viết được phát ngay sang Make.com trước khi Vercel đóng băng Serverless instance.
