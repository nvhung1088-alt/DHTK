...
- **Sprint 6**: Khắc phục lỗi Facebook Pages `CreatePostWithPhotos` trên Make.com: Đã bổ sung thuộc tính `type: 'url'` cho mảng `facebook_photos` khớp schema Make.com, đồng thời nâng cấp thuật toán trích xuất ảnh thông minh (`extractBlogImages`): Phân loại bài viết theo chủ đề (sổ tay, bút viết, túi bóp v.v.), chỉ lọc các sản phẩm CSDL khớp chính xác chủ đề hoặc chèn kho ảnh HD đúng loại sản phẩm từ Unsplash (loại bỏ hoàn toàn việc ghép ảnh quạt/khác chủ đề).
